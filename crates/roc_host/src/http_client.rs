pub fn send_req(roc_request: &roc_http::RequestToAndFromHost) -> roc_http::ResponseToAndFromHost {
    let _request: hyper::Request<String> = match roc_request.to_hyper_request() {
        Ok(r) => r,
        Err(err) => return err.into(),
    };

    // TODO use hyper to make a request
    // and get rid of the higher level reqwest dependency
    todo!()
    ...
    //     Ok(c) => c,
    //     Err(_) => {
    //         return roc_http::ResponseToAndFromHost::bad_status(
    ...
    //     Ok(req) => req,
    //     Err(err) => return roc_http::ResponseToAndFromHost::bad_request(err.to_string().as_str()),
    // };
