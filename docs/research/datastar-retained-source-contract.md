...
The generated ABI needs consuming projections. A borrowed C union accessor
which bit-copies an owning list and callable is not a production move API.
After tag validation, `take_continue` must consume the result shell and leave
one statically obvious owner for the item and machine. Before extraction,
`drop_step` whole-drops any returned branch. Fixed generated wrappers remain
the authority for dropping machines, items, errors, and complete steps.
...
- named consuming result projection rather than borrowed owning-field copies;
...
- callback, byte-budget, item, machine, frame, encoder, request, and waiter
  accounting all returning to zero; and
