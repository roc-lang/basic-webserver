...
use crate::request_parts::request_target;
...
impl RequestMetadataLimits {
    ...
    pub(crate) fn admit<B>(
        &self,
        request: &hyper::Request<B>,
    ) -> Result<(), RequestMetadataRejection> {
        if request_target(request.method(), request.uri()).len() > self.max_target_bytes {
            return Err(RequestMetadataRejection::TargetTooLong {
                limit: self.max_target_bytes,
            });
        }

        if request.headers().len() > self.max_header_fields
            || decoded_header_list_bytes(request.headers()) > self.max_header_list_bytes
        {
...
mod tests {
    ...
    fn target_limit_is_inclusive() {
        ...
        let exact = hyper::Request::builder().uri("/abc").body(()).unwrap();
        let over = hyper::Request::builder().uri("/abcd").body(()).unwrap();

        assert_eq!(limits.admit(&exact), Ok(()));
        assert_eq!(
            limits.admit(&over),
    ...
    fn decoded_header_limit_is_inclusive_and_counts_repeated_fields() {
        ...
        assert_eq!(limits.admit(&exact), Ok(()));
        assert!(matches!(
            limits.admit(&over_bytes),
            Err(RequestMetadataRejection::HeadersTooLarge { .. })
        ));
        assert!(matches!(
            limits.admit(&over_fields),
            Err(RequestMetadataRejection::HeadersTooLarge { .. })
        ));
    }
}
