...
impl RequestMetadata {
    pub(crate) fn validate<B>(request: &hyper::Request<B>) -> Result<Self, InvalidRequestTarget> {
        let target = classify_target(request.method(), request.version(), request.uri())?;
        let uri_authority = request
            .uri()
            .authority()
            .map(|authority| parse_authority(authority.as_str(), AuthoritySource::Uri))
            .transpose()?;
        let host = parse_host_header(request.headers())?;

        let effective_authority = match request.version() {
            Version::HTTP_11 => {
            ...
            Version::HTTP_2 => {
                ...
                if host.present {
                    match (uri_authority, host.authority) {
                        (Some(uri), Some(host)) => {
                            if !authorities_equal(uri, host, request.uri(), request.headers()) {
                                return Err(InvalidRequestTarget);
                            }
                        }
