...
days_in_month = |year, month|
    if List.contains([1, 3, 5, 7, 8, 10, 12], month) {
        31
    } else if List.contains([4, 6, 9, 11], month) {
        30
    } else if month == 2 {
...
normalize_datetime = |current| {
    ...
    if current.day < 1 {
        normalize_datetime({
            ..current,
            year: if current.month == 1 { current.year - 1 } else { current.year },
            month: if current.month == 1 { 12 } else { current.month - 1 },
