...
escape_html_bytes = |value, escape_quotes| {
	escaped_bytes = 
		Str.to_utf8(value).fold(
			...
			|bytes, byte|
				if byte == 38 {
					bytes.concat([38, 97, 109, 112, 59])
				} else if byte == 60 {
					bytes.concat([38, 108, 116, 59])
				} else if byte == 62 {
					bytes.concat([38, 103, 116, 59])
				} else if escape_quotes and byte == 34 {
					bytes.concat([38, 113, 117, 111, 116, 59])
				} else if escape_quotes and byte == 39 {
					bytes.concat([38, 35, 51, 57, 59])
				} else if escape_quotes and byte == 10 {
					bytes.concat([38, 35, 49, 48, 59])
				} else if escape_quotes and byte == 13 {
					bytes.concat([38, 35, 49, 51, 59])
				} else {
					bytes.append(byte)
				},
		)
