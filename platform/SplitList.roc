	...
	walk_help_find_starts = |input_list, separator_list|
		if input_list == [] or separator_list == [] {
			...
			|all_markers, _, idx| {
				...
				if List.sublist(input_list, { start: idx, len }) == separator_list {
					all_markers
						.append(Stop(idx))
		...
		go = |remaining_markers, state|
			match remaining_markers {
				...
				[Start(start), Stop(stop), .. as rest] =>
					go(rest, state.append(List.sublist(input, { start, len: stop - start })))

				[Start(start)] if start >= input.len() => state
				[Start(start)] =>
					state.append(List.sublist(input, { start, len: input.len() - start }))
